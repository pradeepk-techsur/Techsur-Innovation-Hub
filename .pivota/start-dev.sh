...
#
# Agent-synthesis note (multi-match: compose + react-next):
#   This project matches both the `compose` and `react-next` catalog entries —
#   docker-compose.yml (postgres DB + Next.js app service) + package.json with
#   next@^15. Per D-06, multi-match triggers agent fallback.
#
#   Resolution: docker compose up --build is the canonical exec for this stack.
#   The compose file already encodes the full orchestration:
#     db (postgres, healthcheck-gated) → app (migrate → seed → next dev)
#   From the wrapper's perspective this is a single foreground process (compose
#   catalog entry pattern — multi-process-template.md §6). The wrapper's install/
#   lock-file branch is a no-op; compose's own layer cache handles rebuild
#   efficiency. Per-service 0.0.0.0 binding is declared in compose `ports:`
#   (3000:3000) — no env-var lever needed here.
#
#   DB migration ordering: migrate → seed run inside the app service command
#   (AFTER npm ci in the Dockerfile RUN layer) — correct ordering, no wrapper
#   intervention needed. Never duplicate migrate in the wrapper (would run before
#   npm ci, so no node_modules — `tsx: not found`).
...
# block is intentionally EMPTY for this compose-backed entry.
#
# The app service ports mapping is `3000:3000` (0.0.0.0 implicit) — sandbox
# preview reachability confirmed. No loopback-bound ports detected.
...
# No pre-exec snippet required for compose stacks. Docker manages image builds
# (Dockerfile: node:20-alpine + npm ci + COPY). The compose app service command
# (migrate → seed → dev) runs inside the container after npm ci completes —
# correct migrate ordering (post-install, never before).

# === D-12: idempotent install via lockfile hash + presence check ===
# Compose entry: no host-side lockfile or install command. Docker layer cache
# handles rebuild efficiency. LOCK_FILE_PATH and INSTALL_CMD are empty;
...
#
# `docker compose up --build` is a single foreground process (compose catalog
# entry pattern). Compose owns multi-service orchestration, signal propagation,
# and per-service log prefixing. The multi-process-template.md is NOT used here
# (multi-process-template.md §6: compose fast path).
