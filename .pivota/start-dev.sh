...
#   - Agent-synthesized: project matches BOTH react-next AND compose catalog
#     entries (multi-match → D-06 agent fallback). Exec is `docker compose up --build`
#     because the repo ships a fully-specified docker-compose.yml with a
#     health-gated postgres DB, migrate→seed→serve ordering in the app command,
#     and a published port 3000:3000. Compose owns the infrastructure; the wrapper
#     is minimal (no separate npm install — the Dockerfile handles it inside the image).
...
# block is intentionally EMPTY for this compose-based project.
#
# The project's docker-compose.yml already declares:
#   ports: ["3000:3000"]   (app service)
# which binds to 0.0.0.0 on the sandbox host (docker default for bare port mappings).
# No loopback-only mapping (127.0.0.1:3000:3000) was detected — preview is reachable.
...
# No pre-exec snippet for compose-based projects — docker compose up --build
# handles image builds, dependency install (via the Dockerfile's npm ci), and
# service orchestration. No heavyweight one-time installs are needed here.

# === D-12: idempotent install via lockfile hash + presence check ===
# For compose projects: no separate npm install in the wrapper. The Dockerfile
# runs `npm ci` inside the image build, and docker's layer cache provides
# idempotency. LOCK_FILE is intentionally empty — the wrapper proceeds straight
# to EXEC_CMD. docker compose up --build re-evaluates its own build cache.
SENTINEL="/tmp/pivota-setup-sentinel"
LOCK_FILE_PATH=""
INSTALL_PRESENCE_CHECK=""
INSTALL_CMD=''

# No lockfile → no install sentinel needed for compose projects.
# docker compose up --build manages its own build cache internally.
